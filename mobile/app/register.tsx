...
export default function RegisterScreen() {
  ...
  const sendPhoneOtp = async () => {
    ...
    try {
      ...
      // Require native `signInWithPhoneNumber` in this app build.
      if (auth && typeof (auth as any).signInWithPhoneNumber === "function") {
        const confirmation = await (auth as any).signInWithPhoneNumber(
          formData.phone,
        );
        setVerificationId(confirmation as any);
        setPhoneOtpSent(true);
        Alert.alert("OTP Sent", "A verification code was sent to your phone.");
      } else {
        Alert.alert(
          "Error",
          "Phone auth is not available in this build. Create a dev-client or use a web build.",
        );
      }
    } catch (err: any) {
  ...
  const verifyPhoneOtp = async () => {
    ...
    try {
      ...
      if (typeof verificationId === "string") {
        try {
          const authLib = require("@react-native-firebase/auth");
          const credential = authLib.PhoneAuthProvider.credential(
            verificationId,
            enteredPhoneOtp,
          );
          // `auth` is the initialized instance exported from our firebase helper
